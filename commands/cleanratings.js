...
async function deleteRatingsRows(
  rowNumbers
) {
  if (
    !rowNumbers ||
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'Remove clearly invalid rows from the Ratings sheet.'
    )
    .addStringOption(option =>
      option
        .setName('mode')
        .setDescription(
          'What type of invalid rows to remove'
        )
        .setRequired(true)
        .addChoices(
          {
          ...
          {
            name: 'Missing Discord member only',
            value: 'missing_member',
          },
          {
            name: 'All clear invalid rows',
            value: 'all_invalid',
          }
        )
    ),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      for (
        ...
        if (
          ...
          traineeRows.push({
            ...
            rowName,
            reason:
              `Linked to trainee ${member.user.tag}`,
          });
        }
        ...
        if (
          ...
          exSkiraRows.push({
            ...
            rowName,
            reason:
              `Linked to Ex Skira ${member.user.tag}`,
          });
        }
      }
      ...
      if (
        ...
        return interaction.editReply({
          content:
            'No matching invalid Ratings rows were found for that mode.',
          allowedMentions: {
            users: [],
          },
        });
      ...
      await sendLog(
        ...
        [
          ...
          `Deleted Rows:\n- ${truncateList(
            rowsToDelete.map(
              row =>
                `Row ${row.rowNumber} | ${row.rowName} | ${row.reason}`
            ),
            25
          ).join('\n- ')}`,
        ].join('\n')
      ...
      console.error(
        'Error in /cleanratings:',
        error
      );

      return interaction.editReply({
        content:
          '❌ There was an error while running /cleanratings.',
        allowedMentions: {
          users: [],
        },
      });
