...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'Audit invalid or suspicious rows in the Ratings sheet.'
    ),
    ...
    try {
      ...
      for (
        ...
        for (
          ...
          if (
            !memberByCandidate.has(
              candidate
            )
          ) {
            memberByCandidate.set(
              candidate,
              []
            );
          }
      ...
      for (
        ...
        if (!rowDiscordId) {
          noDiscordIdRows.push(
            `Row ${rowNumber} | ${rowName}`
          );
          ...
          if (
            ...
            noDiscordIdNoSafeMatchRows.push(
              `Row ${rowNumber} | ${rowName} | No safe Discord match`
            );
          } else if (
            matches.size > 1
          ) {
            noDiscordIdNoSafeMatchRows.push(
              `Row ${rowNumber} | ${rowName} | Ambiguous Discord match (${matches.size})`
            );
          }
        ...
        if (!member) {
          missingMemberRows.push(
            `Row ${rowNumber} | ${rowName} | Discord ID ${rowDiscordId}`
          );
        ...
        if (
          ...
          traineeRows.push(
            `Row ${rowNumber} | ${rowName} | ${member.user.tag} (${member.id})`
          );
        }
      ...
      for (
        ...
        discordIdCounts.entries()
      ) {
        if (
          entries.length > 1
        ) {
          duplicateDiscordIdRows.push(
            `Discord ID ${discordId} appears in rows: ${
              entries
                .map(
                  entry =>
                    `${entry.rowNumber} (${entry.rowName})`
                )
                .join(', ')
            }`
          );
      ...
      console.error(
        'Error in /auditratings:',
        error
      );

      return interaction.editReply({
        content:
          '❌ There was an error while running /auditratings.',
        allowedMentions: {
          users: [],
        },
      });
